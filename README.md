...
1. **Endpoint Agent (`monitor.py`)**: A standalone Python agent that runs in the background of target workstations. It collects process attributes, evaluates file-path structures for anomalies, logs system statistics, and securely forwards snapshots using endpoint-specific API keys.
...
│
├── frontend/                 # React Single Page Admin Dashboard
...
│
├── monitor.py                # Standalone behavioral endpoint telemetry agent (Python)
...
5. Run the telemetry agent (`monitor.py`) on your endpoint:
   ```bash
   # Make sure dependencies are installed in your monitor environment:
   pip install psutil requests

   # Execute the monitoring loop
   python monitor.py --server http://localhost:5000 --api-key ag-<your_copied_api_key>
   ```
